//
//  AboutCycleLengthViewController.swift
//  PeriodTracker
//
//  Created by Mahdiar  on 8/15/17.
//  Copyright © 2017 Mahdiar . All rights reserved.
//

import UIKit

class AboutCycleLengthViewController: UIViewController , UIPickerViewDelegate , UIPickerViewDataSource {
    ...
    @IBAction func okClicked(_ sender: Any) {
        ...
        if showWorldNormalValue {
            delegate.period(length: 28)
        }else{
            let index = aboutTimesPicker.selectedRow(inComponent: 0)
            delegate.period(length: index + min)
        }
